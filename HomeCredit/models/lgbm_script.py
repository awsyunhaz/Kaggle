...
def write_to_csv(df,preds,file_name="submission.csv"):
    df['TARGET'] = preds
    df[['SK_ID_CURR', 'TARGET']].to_csv(file_name, index= False)
...
def kfold_lightgbm(df, num_folds, stratified = False, debug= False):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df[feats], train_df['TARGET'])):
        ...
        sub_pred = clf.predict_proba(test_df[feats], num_iteration=clf.best_iteration_)[:, 1]
    ...
    write_to_csv(train_df,oof_preds,"oof_lgbm.csv")
    write_to_csv(test_df,sub_preds,"submission_lgbm.csv")
