...
def kfold_et(df, num_folds, stratified = False, debug= False):
    ...
    write_to_csv(train_df,oof_preds,"oof_et.csv")
    write_to_csv(test_df,sub_preds,"submission_et.csv")
...
def main(debug = False):
    ...
    with timer("Process credit card balance"):
        ...
        del cc
        gc.collect()
    with timer("Run ExtraTrees"):
        kfold_et(df, num_folds=5, stratified=False, debug=debug)
